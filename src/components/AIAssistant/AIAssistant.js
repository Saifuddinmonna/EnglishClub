...
import { ERROR_MESSAGES, SUCCESS_MESSAGES, UI_CONFIG } from '../../config';
...
const AIAssistant = () => {
  ...
  const testAI = async () => {
    try {
      ...
      setAiStatus('error');
      setError(ERROR_MESSAGES.AI_CONNECTION_FAILED);
    }
  };

  const validateInput = () => {
    if (mode !== 'practice' && (!input || input.length < UI_CONFIG.minInputLength)) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.message || ERROR_MESSAGES.AI_SERVICE_UNAVAILABLE);
        ...
        <div className="text-center mb-12">
          ...
          <div className="mt-4">
            <div className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium">
              ...
              {aiStatus === 'connected' ? SUCCESS_MESSAGES.AI_CONNECTED :
              ...
              {mode !== 'practice' && (
                <div>
                  ...
                  <p className="text-sm text-gray-500 mt-1">
                    {input.length}/{UI_CONFIG.maxInputLength} characters
                  </p>
                </div>
              )}
